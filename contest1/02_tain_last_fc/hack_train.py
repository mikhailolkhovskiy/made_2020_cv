...
def train(model, loader, loss_fn, optimizer, device):
    ...
    model.eval()
    train_loss = []
    with tqdm.tqdm(loader, total=len(loader), desc="training...") as t:
        for batch in t:
            ...
            t.set_description("Loss: {:5.2}".format(last_loss))
...
def main(args):
    ...
    with open("model3_best.pth", "rb") as fp:
        best_state_dict = torch.load(fp)
    ...
    optimizer = optim.Adam(model.fc.parameters(), lr=args.learning_rate, amsgrad=True)
    loss_fn = fnn.mse_loss
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=2, verbose=True, min_lr=0.00001)
